...
module elevation_fork() {
    ...
    // NOTA: el cube() se centra en Z pero se traslada media altura hacia
    // arriba para que la base del brazo quede exactamente en la cara
    // superior del plato giratorio (turntable_h), no enterrada dentro
    // de la base como en una version anterior de este archivo.
    gap = motor_body_d + 6; // separacion interior entre brazos
    for (side = [-1, 1])
        translate([side * (gap/2 + fork_arm_thick/2), 0, turntable_h + fork_height/2])
...
module rotor_assembly() {
    ...
    translate([0, 0, base_h + 2])
        elevation_fork();
    // Soporte del motor de elevacion: se coloca girado 90 grados sobre
    // el eje Y para que su eje de rotacion quede horizontal (perpendicular
    // al eje de azimut), encajado en el interior de un brazo de la
    // horquilla, cerca de su extremo superior. Este bloque faltaba en
    // una version anterior de este archivo (el modulo estaba definido
    // pero nunca se llamaba).
    gap = motor_body_d + 6;
    translate([-(gap/2 + fork_arm_thick), 0, base_h + 2 + turntable_h + fork_height - motor_body_d/2 - 5])
        rotate([0, 90, 0])
        elevation_motor_bracket();
}
