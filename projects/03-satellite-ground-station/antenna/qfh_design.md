...
La QFH son dos lazos bifilares concéntricos (4 conductores en total) de tamaño ligeramente distinto — un lazo grande y uno pequeño — conectados en paralelo en el punto de alimentación superior. La diferencia de tamaño entre lazos hace que la corriente sea inductiva en uno y capacitiva en el otro; conectados en paralelo, el resultado es una impedancia de alimentación **cercana a 50Ω resistivos de forma nativa, sin red de adaptación adicional** (fuente: página de adaptación de John Coppens, ver referencias). Esto simplifica mucho la alimentación: no hace falta stub ni transformador de impedancia, solo resolver el balun.
...
Siguiendo la recomendación de la referencia consultada: alimentar en paralelo ambos lazos en el punto superior (~50Ω resistivo nativo), y añadir un **balun de choque** — varias vueltas del propio cable coaxial alrededor del mástil de soporte, cerca del punto de alimentación — para bloquear corrientes de modo común en la malla del coaxial sin introducir ninguna transformación de impedancia adicional. Es la solución más simple de las descritas por la referencia y no requiere componentes adicionales.
